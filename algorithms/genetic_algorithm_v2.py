...
from plot.draw_cuttings_v2 import draw_cuttings_v2
...
class GeneticAlgorithm:
    ...
    def generate_solution_population_1(self):
        ...
        self.solution_population_1 = self.__generate_solution_population_n(n=1).copy()
    ...
    def crossover(self):
        ...
        for i in range(len(self.solution_cut_patterns)):
            ...
            crossover_point = random.randint(1, len(parent_1["pattern"]) - 1)
    ...
    def choose_the_best(self):
        ...
        :rtype: tuple
        """
        best_genotype = min(self.solution_population, key=self.calculate_genotype_waste)
        desired_waste = self.calculate_genotype_waste(best_genotype)

        # Filter genotypes with the desired waste
        filtered_genotypes = list(
            filter(lambda genotype: self.calculate_genotype_waste(genotype) == desired_waste, self.solution_population))

        # Choose the best result based on the best waste and number of unique cutting patterns
        best_result = min(filtered_genotypes, key=min)

        # Retrieve cutting patterns corresponding to the best result
        best_chromosomes = [self.get_cut_pattern_by_id(chromosome[1]) for chromosome in best_result]

        return best_result, best_chromosomes
    ...
    def draw_cuttings(self, genotype, chromosomes, genotype_waste):
        ...
        :return: None
        """
        draw_cuttings_v2(genotype=genotype,
